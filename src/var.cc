...
KatiVariableNamesVar::KatiVariableNamesVar(StringPiece name, bool all)
    : name_(name), all_(all) {
  SetReadOnly();
  SetAssignOp(AssignOp::COLON_EQ);
}

bool KatiVariableNamesVar::IsFunc(Evaluator*) const {
  return false;
}

void KatiVariableNamesVar::Eval(Evaluator* ev, string* s) const {
  ConcatVariableNames(ev, s);
}

StringPiece KatiVariableNamesVar::String() const {
  return name_;
}

void KatiVariableNamesVar::ConcatVariableNames(Evaluator* ev, string* s) const {
  WordWriter ww(s);
  vector<StringPiece>&& symbols =
...
string KatiVariableNamesVar::DebugString() const {
  return "*KatiVariableNamesVar*";
}
